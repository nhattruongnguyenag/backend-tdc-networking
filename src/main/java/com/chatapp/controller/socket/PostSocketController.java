...
public class PostSocketController {
    ...
    public List<PostSearchResponseDTO> getPostsByCode(@DestinationVariable("code") String code , @DestinationVariable("userLogin") Long userLogin) {
        return postService.findAllByGroupCode(code,userLogin);
    }

    @MessageMapping({ "/posts/group/{code}/like", "/posts/group/{code}/like/"})
    @SendTo({ "/topic/posts/group/{code}", "/topic/posts/group/{code}" })
    public List<PostSearchResponseDTO> saveMessage(@DestinationVariable("code") String code ,@RequestBody LikeRequestDTO likeRequestDTO) {
    ...
    @SendTo({ "/topic/posts/group/{code}", "/topic/posts/group/{code}/" })
    public List<PostSearchResponseDTO> userSavePostInGroupScreen(@RequestBody UserSavePostRequestDTO userSavePostRequestDTO , @DestinationVariable("code") String code) {
        postService.userSavePost(userSavePostRequestDTO);
